    ...
    public static List<Session> getSession(Project project, boolean cache) {
        ...
        if (httpResponse.getStatusCode() == 200) {
            ...
            if (URLUtils.isCn()){
                ...
                if (user == null) {
                    // navigator panel not up yet (e.g. project still initializing): the progress
                    // REST call above already succeeded and defSession is fully populated, so only
                    // skip the CN userSessionProgress GraphQL refinement below — defSession/sessionList
                    // must still be populated further down, they are real data, not a failure.
                    LogUtils.LOG.warn("getSession: navigator panel unavailable, skip CN userSessionProgress refinement");
                } else {
                    HttpResponse sessionHttpResponse = Graphql.builder().cn(URLUtils.isCn()).operationName("userSessionProgress")
                            .variables("userSlug", user.getUserSlug())
